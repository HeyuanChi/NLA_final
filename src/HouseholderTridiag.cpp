...
void householderTridiag(const arma::cx_mat& A, arma::cx_mat& Q, TMatrix& T)
{
    ...
    {
        // 1) Extract the portion to be "eliminated" in column k
        ...
        // 3) Apply the Householder matrix to R (left and right)
        cx_mat RsubL = R.submat(k+1, k, n-1, n-1);
        RsubL -= 2.0 * (v * (v.t() * RsubL));
        R.submat(k+1, k, n-1, n-1) = RsubL;

        cx_mat RsubR = R.submat(k, k+1, n-1, n-1);
        RsubR -= 2.0 * (RsubR * v) * v.t();
    ...
    // 5) Copy the diagonal and subdiagonal of R (real part) into T
    for (uword i = 0; i < n; ++i) {
        T.diag(i) = R(i, i).real();
        if (i < n - 1) {
